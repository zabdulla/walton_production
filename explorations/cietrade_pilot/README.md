...
`src/cietrade_poll.py --rebuild` (run by hand on a Mac; the scheduled poller now runs in
GitHub Actions without it) rebuilds `out/` and copies the standalone page to
`config.LIVE_PAGE_COPY`, a OneDrive folder. The dashboard's Live card has replaced this as the
phone view. `out/walton_daily_pilot.html` is the fragment published as the artifact
