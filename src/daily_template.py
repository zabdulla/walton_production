"""
Page template for the daily production dashboard (docs/daily.html).

Extracted from build_daily_dashboard.py so the ~1,000-line page shell
(CSS/HTML/JS with embedded JSON data) and the data-prep pipeline can
evolve independently. The builder prepares the JSON payloads;
build_dashboard_html() assembles the page.
"""
...
{THEME_INIT_JS}
    <style>
        :root {{
            --primary: #111827;
            --accent: #0b6e4f;          /* brand evergreen (= lead chart color) */
        ...
        * {{ box-sizing: border-box; }}
        body {{
            ...
            background: linear-gradient(180deg,#edf4f0 0,var(--bg) 360px) fixed;
            color: var(--primary);
        }}
        header {{
            margin-bottom: 24px;
        ...
        .kpi-value, .detail-stat-value {{ font-variant-numeric: tabular-nums; }}
        button {{ transition: background .15s ease, color .15s ease, border-color .15s ease,
                 transform .1s ease; }}
        button:active {{ transform: scale(.97); }}
        @keyframes rise {{ from {{ opacity:0; transform:translateY(8px); }} to {{ opacity:1; transform:none; }} }}
        .kpi-card {{ animation: rise .4s cubic-bezier(.2,.7,.3,1) backwards; }}
        ...
        .kpi-card:nth-child(n+5) {{ animation-delay:.22s }}
        .chart-section, .period-controls {{ animation: rise .45s cubic-bezier(.2,.7,.3,1) backwards; animation-delay:.05s; }}
...
<body>
    <header>
        ...
        <p class="subtitle">View production data by week or month. Click any day for details. Dot markers indicate supervisor notes.</p>
...
{LOCAL_DATE_JS}
        ...
        // Warn when the newest data is old so nobody unknowingly reads a
        // stale dashboard (e.g. the weekly update silently stopped running).
        (function checkStale() {{
            const dates = dailySummary.map(d => d.Date_Str).sort();
            if (!dates.length) return;
            const latestStr = dates[dates.length - 1];
            const ageDays = Math.floor((Date.now() - new Date(latestStr + 'T00:00:00').getTime()) / 86400000);
            if (ageDays > 9) {{
                const b = document.getElementById('staleBanner');
                b.textContent = '⚠ Data may be stale: latest entry is ' + latestStr +
